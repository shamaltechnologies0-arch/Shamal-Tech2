...
export const shamalSeed = async ({
  ...
  const sectorsData = sectorNames.map((name, index) => ({
    name,
    slug: sectorSlugs[index],
    description: `Comprehensive drone and geospatial solutions for the ${name} sector in Saudi Arabia.`,
    useCases: [
      {
  ...
  if (!existingSectors || !existingSectors.sectors || existingSectors.sectors.length === 0) {
    ...
    payload.logger.info('✓ Seeded Sectors Content')
  } else {
    payload.logger.info('✓ Sectors Content already exists')
  }
  ...
  if (!existingSEOSettings || !existingSEOSettings.primaryKeywords) {
    await payload.updateGlobal({
      slug: 'seo-settings',
      data: {
        primaryKeywords: [
    ...
    payload.logger.info('✓ Seeded SEO Settings')
  } else {
    payload.logger.info('✓ SEO Settings already exists')
  }
